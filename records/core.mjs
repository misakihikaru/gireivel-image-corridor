...
export function matches(record,q) {
  ...
  return (!q.medium||record.medium===q.medium)&&(!q.persona||record.persona===q.persona)&&(!q.from||day>=q.from)&&(!q.to||day<=q.to)&&(!q.q||normalize([record.text,record.publicNote,record.alt,names[record.persona]].join(' ')).includes(normalize(q.q)));
}
...
export function validRecord(r) {
  return r&&/^\d{10,25}$/.test(r.id)&&Object.hasOwn(names,r.persona)&&['text','image'].includes(r.medium)&&Number.isFinite(Date.parse(r.publishedAt))&&typeof r.text==='string'&&typeof r.publicNote==='string'&&typeof r.alt==='string'&&(!r.image||/^data\/assets\/[a-f0-9]{64}\.png$/.test(r.image));
}
