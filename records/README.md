...
新しい制作のholdは、作品とは別のmedium=holdとして日時(recordedAt)、人格、公開用の英語と日本語訳だけを掲載する。idはhold-に記録時刻のミリ秒を付けた安定ID。Xへのリンクを付けない。見送り文が空、または編集検査で却下された場合は事実だけを表示する。作品のみ／見送りで絞り込み可能。公開済みの作品・旧データとの互換性を保つ。過去の内部メモの遡及公開はしない。
